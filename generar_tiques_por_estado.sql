-- Función mejorada para generar tiques por estado
...
-- Ejemplo de uso:
-- SELECT * FROM generar_tiques_por_estado(1, 'TIQ', '/tmp/tiques_sorteo1.csv');

-- Para agregar los datos directamente a la tabla participantes sin archivo:
-- SELECT * FROM generar_tiques_por_estado(1, 'TIQ', NULL);

-- Función de ayuda para convertir datos al formato para importar
CREATE OR REPLACE FUNCTION public.obtener_datos_importacion_participantes(
    p_sorteo_id INTEGER,
...
BEGIN
    ...
    SELECT 
        ...
        cod_municipio || ',' ||
        cod_parroquia || ',' ||
        telefono || ',,' ||  -- espacio vacío para email
        p_prefijo || ',' ||  -- prefijo
        ROW_NUMBER() OVER (ORDER BY RANDOM()) || ',' || -- numero_tique
        p_prefijo || '-' || LPAD(cod_estado, 2, '0') || '-' || LPAD((ROW_NUMBER() OVER (ORDER BY RANDOM()))::TEXT, 5, '0')  -- codigo_tique
    FROM 
        public.re_723;
END;
$$ LANGUAGE plpgsql;

-- Ejemplo para obtener datos en formato CSV para importar:
-- COPY (SELECT * FROM obtener_datos_importacion_participantes(1, 'TIQ')) TO '/tmp/participantes_importar.csv';

-- Función para verificar la distribución de tiques
CREATE OR REPLACE FUNCTION public.verificar_distribucion_tiques(
    p_sorteo_id INTEGER
)
RETURNS TABLE (
    estado VARCHAR,
...
BEGIN
    RETURN QUERY
    WITH total AS (
        SELECT COUNT(*) AS total_count FROM participantes WHERE sorteo_id = p_sorteo_id
    )
    SELECT 
        p.estado,
        COUNT(*) AS cantidad,
        (COUNT(*) * 100.0 / total.total_count) AS porcentaje
    FROM 
        participantes p
    JOIN 
        total ON 1=1
    WHERE 
        p.sorteo_id = p_sorteo_id
    GROUP BY 
        p.estado, total.total_count
    ORDER BY 
        cantidad DESC;
END;
$$ LANGUAGE plpgsql; 