...
BEGIN
    ...
    -- Verificar la función para generar tiques
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc 
        WHERE proname = 'generar_tiques_desde_distribucion' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    ) THEN
        RAISE NOTICE 'La función generar_tiques_desde_distribucion no existe. Debe crearse usando el script distribucion_tiques.sql';
    ELSE
        RAISE NOTICE 'La función generar_tiques_desde_distribucion ya existe';
    END IF;
