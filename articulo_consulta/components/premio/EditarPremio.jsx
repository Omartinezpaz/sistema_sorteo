...
const EditarPremio = () => {
  const { sorteoId, premioId } = useParams();
  ...
  // Cargar datos del premio, sorteo y categorías
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const premioResponse = await premioService.getById(premioId);
        if (premioResponse.success && premioResponse.premio) {
          const premioDatos = premioResponse.premio;
          setPremio({
            ...premioDatos,
            fecha_entrega: premioDatos.fecha_entrega ? new Date(premioDatos.fecha_entrega) : null
          });
        } else {
          setError('No se pudo cargar la información del premio');
          return;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await premioService.update(premioId, premio);
      setSuccess(true);
    ...
    return (
      <Layout>
        <Alert severity="error" sx={{ mb: 2 }}>
          No se pudo cargar la información del premio o sorteo
        </Alert>
        <Button variant="contained" onClick={() => navigate(`/sorteos/${sorteoId}`)}>
          Volver al Sorteo
        </Button>
      </Layout>
    );
      ...
      <Box sx={{ mb: 4 }}>
        ...
        {success && (
          <Alert severity="success" sx={{ mb: 2 }}>
            Premio actualizado exitosamente. Redirigiendo...
          </Alert>
        )}
