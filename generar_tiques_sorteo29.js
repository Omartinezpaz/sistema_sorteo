...
async function main() {
  console.log('Iniciando generación de tiques para el sorteo 29...');
  ...
  try {
    ...
    // Verificar distribución para el sorteo 29
    ...
    console.log(`Distribuciones para sorteo 29: ${totalDistribuciones}`);
    
    if (totalDistribuciones === 0) {
      console.log('❌ No hay distribución de tiques configurada para el sorteo 29');
      console.log('Ejecute primero el script verificar_distribucion_sorteo29.js');
      await client.end();
      return;
    }
    
    // Limpiar tiques existentes del sorteo 29
    console.log('\nLimpiando tiques existentes para el sorteo 29...');
    await client.query('DELETE FROM participantes WHERE sorteo_id = 29');
    ...
    if (result.rows.length > 0) {
      ...
      const countResult = await client.query('SELECT COUNT(*) as total FROM participantes WHERE sorteo_id = 29');
